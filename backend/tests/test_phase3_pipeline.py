...
@pytest.mark.asyncio
async def test_recovery_status_endpoint():
    """
    Test Specification 5: GET /api/recovery/{transaction_id}/status
    """
    async with AsyncClient(transport=ASGITransport(app=app), base_url="http://test") as ac:
        res = await ac.get("/api/recovery/txn_4999_upi/status")
...
async def test_human_approval_lifecycle():
    ...
    async with AsyncClient(transport=ASGITransport(app=app), base_url="http://test") as ac:
        ...
        res_exec = await ac.post("/api/recovery/txn_high_value/execute")
...
@pytest.mark.asyncio
async def test_blocked_transaction_lifecycle():
    """
    Test Specification 21 & 22: Blocked retry limit exceeded.
    """
    async with AsyncClient(transport=ASGITransport(app=app), base_url="http://test") as ac:
        res = await ac.post("/api/recovery/txn_retry_exceeded/execute")
        assert res.status_code == 200
        data = res.json()["data"]
...
async def test_idempotent_recovery_execution():
    ...
    async with AsyncClient(transport=ASGITransport(app=app), base_url="http://test") as ac:
        ...
        res_exec2 = await ac.post("/api/recovery/txn_4999_upi/execute")
...
@pytest.mark.asyncio
async def test_full_phase3_end_to_end_recovery():
    """
    Test Specification 1 & 26: Full end-to-end recovery pipeline for txn_4999_upi.
    """
    async with AsyncClient(transport=ASGITransport(app=app), base_url="http://test") as ac:
        # 1. Investigate with AI
        res_an = await ac.post("/api/recovery/txn_4999_upi/analyze")
        ...
        # 2. Execute Recovery
        res_ex = await ac.post("/api/recovery/txn_4999_upi/execute")
        ...
        res_stat = await ac.get("/api/recovery/txn_4999_upi/status")
        assert res_stat.status_code == 200
        assert res_stat.json()["data"]["is_recovered"] is True

        # 5. Verify cryptographic audit chain validity
        res_aud = await ac.get("/api/audit-logs/verify")
